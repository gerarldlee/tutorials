...
//        ViewCompat.setOnApplyWindowInsetsListener(findViewById(R.id.viewWriteFragment), (v, insets) -> {
...
//        } else {
//            LocalBroadcastManager.getInstance(WriteFragment.this).registerReceiver(
...
//        }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        Intent intent = new Intent(view.getContext(), HceCcEmulationServiceVisa.class);

        tvHceServiceLog = getView().findViewById(R.id.tvHceServiceLog);


        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            view.getContext().registerReceiver(mMessageReceiver,
                    new IntentFilter("HceCcEmulatorService"), Context.RECEIVER_NOT_EXPORTED);
        } else {
            LocalBroadcastManager.getInstance(view.getContext()).registerReceiver(
                    mMessageReceiver, new IntentFilter("HceCcEmulatorService"));
        }

        Toast.makeText(view.getContext(), "The HCE Service is started", Toast.LENGTH_SHORT).show();
